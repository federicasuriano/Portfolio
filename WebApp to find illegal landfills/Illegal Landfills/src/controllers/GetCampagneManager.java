...
@WebServlet("/getCampagneManager")
@MultipartConfig
public class GetCampagneManager extends HttpServlet {
...
catch( SQLException se )
{
	...
	public void doGet (HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		List<Campagna> campagne= (List<Campagna>) new ArrayList();
		HttpSession session=req.getSession();
		Manager utente=(Manager) session.getAttribute("utenteCorrente");
		int id= utente.getManagerID();
		String path="/HomeManager.jsp";
		CampagnaDAO campagnaDAO=new CampagnaDAO(connection);
		try {
			campagne=campagnaDAO.listaCampagneManager(id);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		if(campagne!=null) {
		req.setAttribute("campagne", campagne);
